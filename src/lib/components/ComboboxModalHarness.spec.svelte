...
<!--
@component
Test-only host for a `Combobox` inside a `Modal`. The combination is the one a hand-rolled
...
<Modal bind:this={modal} title="Filters">
  <span id="modal-combobox-label">Local</span>
  <Combobox
    bind:value
    {options}
    aria-labelledby="modal-combobox-label"
